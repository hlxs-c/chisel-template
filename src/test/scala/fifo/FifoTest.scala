...
class FifoTest extends AnyFlatSpec with Matchers {
    def testFn[T <: Fifo[_ <: Data]](dut: T) = {
        ...
        println(s"$cnt words in 100 clock, $cycles clock cycles per word")
        assert(cycles >= 0.99, "Cannot be faster than one clock cycle per word")
    }

    "BubbleFifo" should "pass" in {
        simulate(new BubbleFifo(UInt(16.W), 4)) {
            dut => testFn(dut)
        }
    }

    "DoubleBufferFifo" should "pass" in {
        simulate(new DoubleBufferFifo(UInt(16.W), 4)) {
            dut => testFn(dut)
        }
    }

    "RegFifo" should "pass" in {
        simulate(new RegFifo(UInt(16.W), 4)) {
            dut => testFn(dut)
        }
    }

    "MemFifo" should "pass" in {
        simulate(new MemFifo(UInt(16.W), 4)) {
            dut => testFn(dut)
        }
    }
